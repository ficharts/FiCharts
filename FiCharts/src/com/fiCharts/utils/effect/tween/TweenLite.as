...
package com.fiCharts.utils.effect.tween
{
	...
	public class TweenLite 
	{
		...
		public function initTweenVals($hrp:Boolean = false, $reservedProps:String = ""):void 
		{
			...
			else 
			{
				for (p in this.vars) 
				{
					...
					{ //"type" is for TweenFilterLite, and it's an issue when trying to tween filters on TextFields which do actually have a "type" property.
					...
					{ //If we're trying to change the color of a DisplayObject, then set up a quasai proxy using an instance of a TweenLite to control the color.
						...
						{ //In case they're actually trying to remove the colorization, they should pass in null or "" for the tint
					...
					{ //If we're trying to change the volume of a MovieClip or Sound object, then set up a quasai proxy using an instance of a TweenLite to control the volume.
		...
		public static function goto($target:Object, $duration:Number, $vars:Object):TweenLite 
		{
